...
class Uploader {
	...
	/**
	 * The mimetype-to-extension mapping; imported from config.
	 *
	 * @access protected
	 * @var array
	 * @static
	...
	public function importRemote($url, array $options = array()) {
		...
		$file = curl_exec($ch);
		$this->_data[$this->_current]['type'] = curl_getinfo($ch, CURLINFO_CONTENT_TYPE);
	...
	public static function extFromMime($mimetype) {
		if (empty(self::$_exts)) {
			self::$_exts = Configure::read('Uploader.exts');
		}
